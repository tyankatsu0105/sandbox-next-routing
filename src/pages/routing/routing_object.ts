...
export const getReplacedPath = (params: { pathname: RoutingObject['pathname']; path: Record<string, string> }) => {
...
export const createRoutingObject = <Pathname extends RoutingObject['pathname'], QueryParameters extends RoutingObject['queryParameters']>(routingObject: {
  ...
  const queryParameterKeys: QueryParameters[number]['key'][] = routingObject.queryParameters.map(queryParameter => queryParameter.key)
  /**
   * pathを生成する
   * @example
   * const USER = {
  ...
  const generatePath = (parameters: {
    query: {
      [Key in QueryParameters[number]['key']]?: Extract<QueryParameters[number], { key: Key }> extends {
        key: Key
